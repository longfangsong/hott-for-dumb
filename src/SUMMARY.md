# Summary

- [是什么，为什么](./why.md)
- [预备知识](./preliminary-knowledge.md)

# 什么是等于

- [如何描述等于：Identity 类型](./identity-type/identity-type.md)
- [例子：自然数](./natural-number/natural-number.md)
- [函数之间的等于：同伦与函数外延性公理](./homotopy/homotopy.md)
- [类型之间的等于：等价与 Univalence 公理](./equivalence/equivalence.md)

# 一些常见的数学对象

- [命题](./prop/prop.md)
- [集合](./set/set.md)
- [群](./group/group.md)

# 拓扑学与同伦论 I

- [命题以下和集合以上：n-类型](./n-type/n-type.md)
- [Loop space](./loop-space/loop-space.md)

# 范畴论

- [（前）范畴与同构](./category/category.md)
- [米田（Yoneda）引理和泛性质](./universal-property/universal-property.md)

# Circle

- [例子：Circle](./circle/circle.md)

# 附录

## 证明技巧

- [encode-decode 方法](./encode-decode/encode-decode.md)
